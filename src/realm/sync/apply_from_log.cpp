...
public:
    ...
    void parse_download_message(StringView sv);
};

ValueMap parse_args(StringView sv)
{
    ValueMap values;
    if (sv[0] != '(') {
    ...
    while (*p != ')') {
        ...
        while (*p == ' ')
            p++;
        auto equal_pos = StringView(p, sv.end() - p).find('=');
        if (equal_pos == realm::npos)
            break;
        std::string kw = std::string(p, equal_pos);
        p += (equal_pos + 1);
        uint64_t i;
        auto parse_res = realm::util::from_chars(p, sv.end(), i, 10);
        values.emplace(kw, i);
        p = parse_res.ptr;
    }
    return values;
...
Buffer<char> changeset_compressed_to_binary(const char* start, const char* end)
{
    ...
    uncompressed_body_buffer.set_size(decompressed_size);
    std::error_code ec = _impl::compression::decompress(decode_buffer.data(), *decoded_size,
                                                        uncompressed_body_buffer.data(), decompressed_size);

    if (ec) {
        throw MessageParseException("compression::inflate: %1", ec.message());
    }

    return std::move(uncompressed_body_buffer);
}

void DownloadChangeset::parse_download_message(StringView sv)
{
    auto start = sv.begin();
    auto end = strchr(start, ')') + 1;

    values = parse_args(StringView(start, end - start));
    auto pos = end;
...
std::pair<HeaderType, size_t> find_interesting_header(StringView sv)
{
    ...
    while (!input_view.empty()) {
        ...
        switch (type) {
            case HeaderType::DownloadChangeset: {
                ...
                _impl::SimpleInputStream input_stream{downloaded_changeset.changeset_buffer.data(),
            ...
            case HeaderType::Download: {
                ...
                auto end = strchr(start, ')') + 1;
                auto download_header = parse_args(StringView(start, end - start));
                ...
                auto downloadable_bytes = download_header["downloadable_bytes"];
                ...
                if (num_changesets) {
                    ...
                    if (!history.integrate_server_changesets(progress, &downloadable_bytes, changesets.data(),
                                                             num_changesets, version_info, integration_error, *logger,
                                                             nullptr)) {
            ...
            case HeaderType::Upload: {
                auto start = input_view.begin();
                auto end = strchr(start, ')') + 1;
                auto upload_header = parse_args(StringView(start, end - start));
                if (upload_header["num_changesets"] == 1) {
                    ...
                    _impl::SimpleInputStream input_stream{upload_changeset.changeset_buffer.data(),
                                                          upload_changeset.changeset_buffer.size()};
                    sync::Changeset changeset;
                    sync::parse_changeset(input_stream, changeset);
                    // changeset.print();
            ...
            case HeaderType::Ident: {
                auto start = input_view.begin();
                auto end = strchr(start, ')') + 1;
                auto ident_header = parse_args(StringView(start, end - start));

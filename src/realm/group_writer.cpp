    ...
    , m_durability(dura)
{
    ...
    while (top.size() < Group::s_evacuation_point_ndx + 1) {
        top.add(0);
    }
    ...
    m_evacuation_limit = size_t(top.get_as_ref_or_tagged(Group::s_evacuation_point_ndx).get_as_int());
}
...
ref_type GroupWriter::write_group()
{
    ...
    auto logical_size = size_t(top.get_as_ref_or_tagged(Group::s_file_size_ndx).get_as_int());
    int size_per_entry = (logical_size < 0x8000000 ? 8 : 16) + 8;
    ...
    // if we have one block of memory in 'm_under_evacuation'.
    if (m_under_evacuation.size() == 1) {
        auto& elem = m_under_evacuation.back();
        size_t pos = elem.ref;
        REALM_ASSERT(pos + elem.size == logical_size);
        logical_size = util::round_up_to_page_size(pos);
        elem.size = (logical_size - pos);
        top.set(Group::s_file_size_ndx, RefOrTagged::make_tagged(logical_size));
        m_evacuation_limit = 0;
    }
    ...
    if (m_evacuation_limit == 0) {
        size_t used_space = logical_size - m_free_space_size;
        if (m_free_space_size - m_locked_space_size > 2 * used_space) {
            // Clean up potential
            m_evacuation_limit = used_space + used_space / 2 + m_locked_space_size;
